...
function DirectMessage({ name }: { name: string }) {
  ...
  return (
    <div onClick={()=>router.push("/user/messages")} onMouseEnter={()=>control.start({x:"-1.5rem",scale:"100%"})} onMouseLeave={()=>control.start({x:"0",scale:"0"})}  className="w-full relative flex flex-row gap-2 items-center cursor-pointer hover:bg-[#1d1d23] pl-5 py-2">
      ...
      <motion.div className="absolute right-0 scale-0" animate={control} transition={{duration:.5,type:"spring"}}>
        <CiMenuKebab size={10} />
      </motion.div>
    </div>
  );
